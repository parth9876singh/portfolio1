...
const Navbar = ({ navLinks = defaultNavLinks }) => {
  ...
  const activeSection = useScrollSpy(navLinks.map((link) => link.id), 120);
  ...
  const initials = profileName.split(' ').map(n => n[0]).join('');
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center">
          ...
          >
            <div className="text-2xl font-display font-bold text-white relative flex items-center h-10 w-10 justify-center rounded-xl bg-white/5 border border-white/10 group-hover:border-brand-indigo/50 transition-colors">
              ...
              <motion.div 
                className="absolute inset-0 rounded-xl bg-brand-indigo/20 blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300"
              />
            </div>
          </MagneticButton>
        ...
        {/* Scroll Progress Bar */}
        <div className={`absolute bottom-0 left-0 h-[1px] w-full transition-all duration-300 ${isScrolled ? 'bg-gradient-to-r from-brand-indigo via-brand-cyan to-brand-indigo shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'bg-white/5'}`}>
          <motion.div 
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="pt-8"
            >
              <MagneticButton onClick={() => handleScrollTo('contact')} variant="primary">
                Let's Talk
              </MagneticButton>
            </motion.div>
          </motion.div>
