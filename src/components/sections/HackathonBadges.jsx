...
import { Trophy, Code2, Flame, Target, Zap, Medal, Award, Star } from 'lucide-react';
            ...
            <div className="hb-animate pt-6 pb-2 border-t border-white/5 mt-8">
              ...
              <div className="flex flex-wrap gap-6 justify-center md:justify-start">
                {myBadges.map((badge, i) => {
                  ...
                  return (
                    <div key={i} className="filter drop-shadow-[0_8px_16px_rgba(0,0,0,0.4)] transition-all duration-300 hover:-translate-y-2 hover:drop-shadow-[0_12px_24px_rgba(255,255,255,0.05)]">
                      <div className="relative w-[130px] h-[150px]">
                        ...
                        <div 
                          ...
                          style={{ clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)' }}
                        >
                          <Icon size={46} className="mb-3 text-[#2d3b4e] drop-shadow-sm" />
                          <span className="font-bold text-[14px] tracking-wide text-[#2d3b4e]">{badge.name}</span>
                          <div className="flex gap-1.5 mt-2.5">
                            {[...Array(badge.rating)].map((_, idx) => (
                              <Star key={idx} size={10} className="fill-[#2d3b4e] text-[#2d3b4e]" />
                            ))}
                          </div>
                        </div>
                      </div>
        ...
        {activeTab === 'hackathon' && (
          <div className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-start gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-400 mb-1">
                          Organized by <span className="text-white font-medium">{hack.organizer}</span>
                        </p>
                        <p className="text-xs font-mono text-gray-500 mb-4">{hack.date}</p>
            ...
            <div className="hb-animate rounded-2xl p-8 border-2 border-dashed border-white/15 bg-white/3 flex flex-col items-center justify-center text-center gap-3 min-h-40 hover:border-amber-500/30 transition-colors">
              ...
              <p className="text-white/60 font-display font-semibold">More Hackathons Coming</p>
              <p className="text-xs text-gray-500">Actively participating in coding competitions</p>
            </div>
          </div>
        )}
      </div>
