...
import { ExternalLink, Github, ArrowRight, UtensilsCrossed, PenTool } from 'lucide-react';
...
const getGradientPlaceholder = (projectId) => {
  const placeholders = {
    2: { gradient: 'from-orange-600 via-red-500 to-orange-600', icon: UtensilsCrossed, letter: 'V' },
    3: { gradient: 'from-purple-600 via-indigo-500 to-pink-600', icon: PenTool, letter: 'B' },
  };
  return placeholders[projectId] || { gradient: 'from-indigo-600 via-cyan-500 to-indigo-600', icon: null, letter: '?' };
};
...
const RegularProjectCard = ({ project }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`relative w-full h-64 overflow-hidden bg-linear-to-br ${placeholder.gradient}`}>
        {/* Placeholder Content */}
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {IconComponent && <IconComponent size={64} className="text-white/40 mb-4" />}
          <span className="text-8xl font-display font-bold text-white/20">{placeholder.letter}</span>
        </div>
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between z-20">
          ...
          <span className={`text-xs px-3 py-1 rounded-full border font-medium ${project.status === 'Live' ? 'border-green-500/50 text-green-300 bg-green-500/15' : 'border-amber-500/50 text-amber-300 bg-amber-500/15'}`}>
            {project.status}
          </span>
        </div>
      </div>
      ...
      <div className="p-8 relative z-20 bg-gradient-to-b from-white/5 to-brand-base/80">
        ...
        <div className="mb-5 pb-5 border-b border-white/10">
          <p className="text-xs font-mono uppercase tracking-wider text-brand-indigo mb-1">Problem Solved</p>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {(project.techStack || []).map((t, i) => (
            <span key={i} className="text-xs font-mono uppercase tracking-wider text-gray-300 bg-white/10 border border-white/20 px-3 py-1.5 rounded-lg hover:border-brand-indigo/50 transition-colors">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className={`flex ${project.liveURL ? 'gap-3' : 'gap-3'}`}>
          <a href={project.githubURL || '#'} target="_blank" rel="noreferrer" className={project.liveURL ? "flex-1 group/btn" : "w-full group/btn"}>
          ...
          {project.liveURL && (
            <a href={project.liveURL} target="_blank" rel="noreferrer" className="flex-1 group/btn">
...
const Projects = () => {
  ...
  const regularProjects = projects.filter(p => !p.featured && (activeTab === "All" || p.category.toLowerCase().replace(' ', '') === activeTab.toLowerCase().replace(' ', '')));
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-32 px-6 lg:px-12 relative w-full overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="project-header flex flex-col items-center text-center mb-20">
          ...
          <p className="text-gray-300 font-sans max-w-2xl text-lg">
            A selection of my best projects, balancing scalable engineering with premium user experiences.
          </p>
        </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`cursor-none relative px-6 py-3 rounded-full font-medium transition-all text-base ${
                activeTab === cat ? "text-white" : "text-gray-400 hover:text-white"
              }`}
            >
              {activeTab === cat && (
                <motion.div
