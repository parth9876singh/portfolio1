...
import { 
  ...
  SiPostgresql, SiGit, SiGithub, SiPostman 
} from 'react-icons/si';
...
const iconMap = {
  FaJava, SiCplusplus, FaC, SiJavascript, 
  SiReact, SiNodedotjs, SiExpress, 
  SiMongodb, SiMysql, SiPostgresql, SiGit, 
  SiGithub, SiPostman, FaBrain, FaUsers, FaPeopleArrows, FaTasks
};
...
const SkillCard = ({ skill }) => {
  ...
  const baseScale = (skill.level === 'Advanced' || skill.level === 'Expert') ? 1.02 : 1;
    ...
    >
      ...
      >
        ...
        <div 
          ...
          style={{ boxShadow: `0 0 20px ${skill.color}40`, border: `1px solid ${skill.color}80` }}
        />
        ...
        <div className="relative z-10 w-full h-full flex flex-col items-center justify-between">
          ...
          <div className="mt-2 group-hover:scale-110 transition-transform duration-300">
            {IconComponent ? (
              <IconComponent size={60} color={skill.color} className="drop-shadow-lg" />
            ) : (
          ...
          <div className="flex flex-col items-center gap-3 w-full">
            ...
            <span className={`text-[10px] font-mono tracking-wider uppercase px-3 py-1.5 rounded-full border ${getLevelStyles(skill.level)}`}>
              {skill.level}
            </span>
          </div>
...
const Skills = () => {
  ...
  const filteredSkills = skills.filter(skill => 
    activeTab === "All" ? true : skill.category.toLowerCase() === activeTab.toLowerCase()
  );
  ...
  return (
    <section id="skills" className="py-32 px-6 lg:px-12 relative z-10 w-full max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
        ...
        <p className="text-gray-400 max-w-md md:text-right font-sans">
          Curated selection of technologies I use to build scalable, high-performance web applications.
        </p>
      </div>
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`relative px-5 py-2.5 rounded-full text-sm font-medium transition-colors outline-none focus:outline-none ${
              activeTab === cat ? "text-brand-base" : "text-gray-400 hover:text-white glass-card"
            }`}
          >
            {activeTab === cat && (
              <motion.div
