            ...
            >
              ...
              <div className="relative p-8 rounded-2xl border-l-4 border-l-brand-indigo flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-1/3 flex flex-col gap-4">
                  ...
                  <div>
                    <p className="text-xl text-gray-100 font-semibold mb-4">{item.program}</p>
                  </div>
                      ...
                      {item.certified && (
                        item.certificateLink ? (
                          <a 
                            ...
                            className="px-3 py-1 text-xs font-semibold bg-green-500/20 text-green-400 rounded-full border border-green-500/30 flex items-center gap-1 shadow-[0_0_10px_rgba(74,222,128,0.3)] hover:bg-green-500/30 hover:shadow-[0_0_15px_rgba(74,222,128,0.5)] transition-all cursor-pointer"
                          >
                            ✓ Certified
                          </a>
                        ) : (
                          <span className="px-3 py-1 text-xs font-semibold bg-green-500/20 text-green-400 rounded-full border border-green-500/30 flex items-center gap-1 shadow-[0_0_10px_rgba(74,222,128,0.3)]">
                            ✓ Certified
                          </span>
                        )
                      )}
                ...
                <div className="md:w-2/3 flex flex-col gap-6">
                  ...
                  <ul className="space-y-3">
                    {item.achievements.map((achievement, idx) => {
                      ...
                      return (
                        <li key={idx} className="training-bullet flex items-start gap-3 text-gray-300">
                          <CheckIcon />
                          <span>
                            {parts.map((part, i) => 
                              part.match(/30%/) ? (
                                <strong key={i} className="text-brand-cyan font-semibold">{part}</strong>
                              ) : (
                                part
                              )
                            )}
